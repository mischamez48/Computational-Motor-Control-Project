...
def run_experiment(pars):
    ...
    # set spawn mode
    animat_options['spawn']['mode'] = SpawnMode.TRANSVERSE
    animat_options['spawn']['pose'] = pars.animal_pose

    for i, joint_option in enumerate(animat_options["morphology"]["joints"]):
        joint_option.initial[0] = pars.joint_poses[i]
    ...
    if pars.controller == "abstract oscillator":
        controller = AbstractOscillatorController(
            pars
        )
