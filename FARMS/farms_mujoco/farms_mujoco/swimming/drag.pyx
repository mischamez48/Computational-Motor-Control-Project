...
cdef void compute_buoyancy(
    ...
    if mass > 0:
        ...
        tmp[2] = -1000*mass*gravity/density*min(
            0.5*max(surface - position + height, 0)/height,
            1,
        )
...
cpdef bint drag_forces(
    ...
    # Add fluid velocity
    quat_rot(
        vector=water.velocity(pos_x, pos_y, pos_z, links_index),
        quat=global2urdf,
        quat_c=quat_c,
        tmp4=tmp4,
...
cdef class WaterProperties:
    ...
    cdef DTYPEv2 _velocities

    def __init__(self, surface, density, viscosity, velocities):
    ...
    cdef DTYPEv2 velocities(self, double x, double y, double z) nogil:
        """Velocity of all links in global frame"""
        return self._velocities

    cdef DTYPEv1 velocity(self, double x, double y, double z, int ind) nogil:
        """Velocity of a link in global frame"""
        return self._velocities[ind, :]

    cdef double viscosity(self, double x, double y, double z) nogil:
        """Viscosity"""
        return self._viscosity

    cpdef void set_velocities(self, DTYPEv1 vx, DTYPEv1 vy, DTYPEv1 vz):
        """Set velocity of all links """
        self._velocities[:, 0] = vx
        self._velocities[:, 1] = vy
        self._velocities[:, 2] = vz

    cpdef void set_velocity(self, double vx, double vy, double vz, int ind):
        """Set velocity of a link """
        self._velocity[ind, 0] = vx
        self._velocity[ind, 1] = vy
        self._velocity[ind, 2] = vz
...
cdef class SwimmingHandler:
    ...
    def __init__(self, data, animat_options, arena_options, units, physics):
        ...
        self.water = WaterProperties(
            ...
            velocities=np.array(
                np.vstack( [water_options.velocity[:3]]*self.n_links),
                dtype=float
            ),
        )
        ...
        self.heights = np.array([
            [
                physics.model.geom_rbound[geom_i]
    ...
    cpdef void set_water_velocity(self, DTYPEv1 velocity, int links_index):
        """Set water velocity"""
        self.water.set_velocity(vx=velocity[0], vy=velocity[1], vz=velocity[2], ind=links_index)

    cpdef void set_water_velocities(self, DTYPEv2 velocities):
        """Set water velocity"""
        self.water.set_velocities(vx=velocities[:, 0], vy=velocities[:, 1], vz=velocities[:, 2])
