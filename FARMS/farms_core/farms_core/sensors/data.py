...
class SensorsData(SensorsDataCy):
    ...
    def from_options(
        ...
        return cls.from_names(
            buffer_size=simulation_options.n_iterations,
...
class LinkSensorArray(SensorData, LinkSensorArrayCy):
    ...
    def global_com_lin_velocity(self, iteration: int):
        """Global CoM linear velocity"""
        total_mass = 0
        mass_lin_velocity = np.zeros(3)
        for link_i, mass in enumerate(self.masses):
            mass_lin_velocity += mass*self.com_lin_velocity(
                iteration=iteration,
                link_i=link_i,
            )
            total_mass += mass
        assert total_mass > 0, 'No masses'
        mass_lin_velocity /= total_mass
        return mass_lin_velocity

    def global_com_positions_all(self):
        """Global CoM positions across all iterations"""
        return np.array(
            [
                self.global_com_position(iteration)
                for iteration in range(self.size(0))
            ]
        )

    def global_com_lin_velocities_all(self):
        """Global CoM linear velocities across all iterations"""
        return np.array(
            [
                self.global_com_lin_velocity(iteration)
                for iteration in range(self.size(0))
            ]
        )
